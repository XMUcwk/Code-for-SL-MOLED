...
/* allocate device memory for matrices */
	...
	float *d_K = NULL;
    cudaMalloc( (void**) &d_K, SpinMxNum * SpinMxSliceNum * (2*(*TypeNum-1))  * sizeof(float)) ;
	cudaMemcpy( d_K, K, SpinMxNum * SpinMxSliceNum * (2*(*TypeNum-1)) * sizeof(float), cudaMemcpyHostToDevice ) ;
            ...
            *Ext = *(ExtLine+ *Exti);
            ...
            if (*Ext != 0){
				if (g_Sig.size() !=0){
					...
					BlochKernelCESTGPU<<< dimGridImg, dimBlockImg, SBufferLen >>>
				...
				if (*gpuFetch !=0){
					...
					cudaMemcpy( K, d_K, SpinMxNum * SpinMxSliceNum * (2*(*TypeNum-1)) * sizeof(float), cudaMemcpyDeviceToHost );
