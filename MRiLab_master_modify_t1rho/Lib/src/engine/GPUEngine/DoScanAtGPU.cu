...
/* allocate device memory for matrices */
    ...
    float *d_dWRnd = NULL;
	mexPrintf("GPU_MEM3	%d\n",SpinMxNum * SpinMxSliceNum * (*SpinNum) * (*TypeNum) * sizeof(float));
...
/* allocate device memory for GPU execution sequence*/
    float *d_Sig = NULL;
//mexPrintf("MaxutsStep %d\n",MaxutsStep);
//mexPrintf("GPU_MEM4 %d\n",(5+3*(*TxCoilNum)) * MaxutsStep * sizeof(float));
...
/* allocate device memory for buffering acquired signal */
    float *d_Sx = NULL;
	//mexPrintf("GPU_MEM5	%d\n", SpinMxNum * PreSignalLen * (*TypeNum) * (*RxCoilNum) * sizeof(float));
...
/* start simulator execution loop */
	mexPrintf( "Wishing you the best of health! WJG ^_^\n ");
            ...
            if (*Ext != 0){
				if (g_Sig.size() !=0){
					...
					if (PreSignalLen!=SignalLen && SignalLen>0){
						...
						cudaFree(d_Sx);
						cudaFree(d_Sy);
                       // mexPrintf("signallen1	%d\n",SignalLen);
						//mexPrintf("GPU_MEM1	%d\n",SpinMxNum * SignalLen * (*TypeNum) * (*RxCoilNum) * sizeof(float));
		 ...
		 if (*dt <= 0){ 
			if (g_Sig.size() !=0){
				...
				if (PreSignalLen!=SignalLen && SignalLen>0){
					...
					cudaFree(d_Sx);
					cudaFree(d_Sy);
					//mexPrintf("SignalLen2	%d	\n",SignalLen);
					mexPrintf("GPU_MEM2	%d	\n",SpinMxNum * SignalLen * (*TypeNum) * (*RxCoilNum) * sizeof(float));
					cudaMalloc( (void**) &d_Sx, SpinMxNum * SignalLen * (*TypeNum) * (*RxCoilNum) * sizeof(float)) ;
					cudaMalloc( (void**) &d_Sy, SpinMxNum * SignalLen * (*TypeNum) * (*RxCoilNum) * sizeof(float)) ;
					/* zero signal buffer */
					cudaMemset(d_Sx, 0 ,SpinMxNum * SignalLen * (*TypeNum) * (*RxCoilNum) * sizeof(float)); /* only work for 0 */
