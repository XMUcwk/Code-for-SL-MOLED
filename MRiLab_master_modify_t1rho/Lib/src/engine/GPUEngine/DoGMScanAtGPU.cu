...
/************************************************************************
 MEX code for Chemical Exchange Saturation Transfer (CEST) spin discrete evolution using IPP or 
 Framewave and parallel GPU computation (CUDA) written by Fang Liu (leoliuf@gmail.com).
************************************************************************/
...
/* assign pointers */
   Gyro            = (double*) mxGetData(mxGetField(mexGetVariablePtr("global", "VObj"), 0, "Gyro"));
	...
	TypeFlag      = (double*) mxGetData(mxGetField(mexGetVariablePtr("global", "VObj"), 0, "TypeFlag"));
...
/* allocate device memory for matrices */
	...
	double *d_TypeFlag = NULL;
    cudaMalloc( (void**) &d_TypeFlag, *TypeNum * sizeof(double)) ;
	cudaMemcpy( d_TypeFlag, TypeFlag, *TypeNum * sizeof(double), cudaMemcpyHostToDevice ) ;
            ...
            *Ext = *(ExtLine+ *Exti);
            ...
            if (*Ext != 0){
				if (g_Sig.size() !=0){
					...
					BlochKernelGMGPU<<< dimGridImg, dimBlockImg, SBufferLen >>>
										((float)*Gyro, d_CS, d_TypeFlag, d_Rho, d_T1, d_T2, d_K, d_Mz, d_My, d_Mx, d_Buffer,
