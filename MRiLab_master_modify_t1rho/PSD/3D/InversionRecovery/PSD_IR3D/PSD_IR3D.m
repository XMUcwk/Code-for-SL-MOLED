...
% MRiLab Version 1.3                           %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [rfAmp,rfPhase,rfFreq,rfCoil,GzAmp,GyAmp,GxAmp,ADC,Ext,uts,ts,flags]=PSD_IR3D
global VCtl
global VVar
CV1=10e-3;
CV10=0;
...
p.Duplicates=1;
p.FA=180;
p.Notes='180 degree inversion rf';
AttributeOpt={'on','off'};
...
p.rfFreq=0;
p.rfPhase=0;
p.tEnd=VCtl.TI+1.1e-3;
p.tStart=VCtl.TI+0.1e-3;
if strcmp(p.Switch,'on')
if strcmp(p.AnchorTE,'on')
switch VCtl.TEAnchor
case 'Start'
...
p.t2End=VCtl.TI+1.1e-3;
p.t2Start=VCtl.TI+0.1e-3;
p.tRamp=40e-6;
if strcmp(p.Switch,'on')
...
p.Switch=AttributeOpt{1};
p.t1End=VCtl.TI+CV2;
p.t1Start=VCtl.TI+CV1;
p.t2End=VCtl.TI+VCtl.TE+CV2;
p.t2Start=VCtl.TI+VCtl.TE+CV1;
p.tRamp=100e-6;
if strcmp(p.Switch,'on')
[GzAmp2,GzTime2]=GzCartesian(p);
GzAmp=[GzAmp GzAmp2];
