...
% MRiLab Version 1.3                           %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [rfAmp,rfPhase,rfFreq,rfCoil,GzAmp,GyAmp,GxAmp,ADC,Ext,uts,ts,flags]=PSD_SE3DEPI
global VCtl
global VVar
CV1=10e-3;
CV10=0;
...
p.rfFreq=0;
p.rfPhase=0;
p.tEnd=0.5e-3+VCtl.TE/2+1e-3;
p.tStart=0.5e-3+VCtl.TE/2-1e-3;
if strcmp(p.Switch,'on')
if strcmp(p.AnchorTE,'on')
switch VCtl.TEAnchor
case 'Start'
...
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.t1Start=0.5e-3+VCtl.TE/2+3e-3;
p.t2Middle=VCtl.TE;
if strcmp(p.Switch,'on')
...
p.tStart=(0.5e-3+VCtl.TE/2-1e-3)/2;
if strcmp(p.Switch,'on')
