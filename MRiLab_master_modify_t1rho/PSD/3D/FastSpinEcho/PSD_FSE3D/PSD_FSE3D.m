...
% MRiLab Version 1.3                           %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [rfAmp,rfPhase,rfFreq,rfCoil,GzAmp,GyAmp,GxAmp,ADC,Ext,uts,ts,flags]=PSD_FSE3D
global VCtl
global VVar
CV1=1e-3;
CV10=0;
...
p.rfFreq=0;
p.rfPhase=pi/2;
p.tEnd=CV9+VCtl.FSE_ESP/2+0.4e-3;
p.tStart=CV9+VCtl.FSE_ESP/2;
if strcmp(p.Switch,'on')
if strcmp(p.AnchorTE,'on')
switch VCtl.TEAnchor
case 'Start'
...
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.t1End=VCtl.FSE_ESP-CV1;
p.t1Start=VCtl.FSE_ESP-CV2;
p.t2End=VCtl.FSE_ESP+CV2;
p.t2Start=VCtl.FSE_ESP+CV1;
p.tRamp=100e-6;
if strcmp(p.Switch,'on')
