...
% MRiLab Version 1.3                           %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [rfAmp,rfPhase,rfFreq,rfCoil,GzAmp,GyAmp,GxAmp,ADC,Ext,uts,ts,flags]=PSD_OLED4_MS
global VCtl
global VVar
CV1=75e-3;
CV10=10.35e-3;
...
p.rfFreq=0;
p.rfPhase=0;
p.tEnd=CV4+VCtl.delta_TE*4+1.5e-3;
p.tStart=CV4+VCtl.delta_TE*4-1.5e-3;
if strcmp(p.Switch,'on')
if strcmp(p.AnchorTE,'on')
switch VCtl.TEAnchor
case 'Start'
...
p.rfFreq=0;
p.rfPhase=0;
p.tEnd=CV4+8*VCtl.delta_TE+1.5e-3;
p.tStart=CV4+8*VCtl.delta_TE-1.5e-3;
if strcmp(p.Switch,'on')
if strcmp(p.AnchorTE,'on')
switch VCtl.TEAnchor
case 'Start'
...
p.rfFreq=0;
p.rfPhase=0;
p.tEnd=CV4+12*VCtl.delta_TE+1.5e-3;
p.tStart=CV4+12*VCtl.delta_TE-1.5e-3;
if strcmp(p.Switch,'on')
if strcmp(p.AnchorTE,'on')
switch VCtl.TEAnchor
case 'Start'
...
p.rfFreq=0;
p.rfPhase=0;
p.tEnd=CV4+VCtl.TE+1.4975e-3;
p.tStart=CV4+VCtl.TE-1.4975e-3;
if strcmp(p.Switch,'on')
if strcmp(p.AnchorTE,'on')
switch VCtl.TEAnchor
case 'Start'
...
p=[];
%--------------------
p.Area=-3.1*1/VCtl.RFreq;
p.DupSpacing=0;
p.Duplicates=1;
p.Notes=[];
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{2};
p.tEnd=CV12+2e-3+3e-3+2*CV14;
p.tStart=CV12+2e-3+2*CV14;
if strcmp(p.Switch,'on')
[GzAmp6,GzTime6]=GxAreaTrapezoid(p);
GzAmp=[GzAmp GzAmp6];
...
p.Switch=AttributeOpt{1};
p.t1Start=VCtl.TR;
p.t2Middle=VCtl.TE+0.5*VCtl.EPI_ESP*VCtl.EPI_ETL+CV8+CV5+0.5e-3;
if strcmp(p.Switch,'on')
...
p=[];
%--------------------
p.Area=-3.1*VCtl.ResPhase/VCtl.EPI_ETL/VCtl.EPI_ShotNum/VCtl.RPhase;
p.DupSpacing=0;
p.Duplicates=1;
p.Notes=[];
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.tEnd=CV4+VCtl.TE-2.5e-3+0.5e-3;
p.tStart=CV4+VCtl.TE-2.5e-3-0.5e-3;
if strcmp(p.Switch,'on')
...
p.Switch=AttributeOpt{1};
p.t1Start=VCtl.TR;
p.t2Middle=VCtl.TE+1.5*VCtl.EPI_ESP*+VCtl.EPI_ETL+CV8+2.358e-3+CV5+CV7;
if strcmp(p.Switch,'on')
...
p=[];
%--------------------
p.Area=-128/128*1/VCtl.RFreq;
p.DupSpacing=0;
p.Duplicates=1;
p.Notes=[];
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.tEnd=VCtl.TE+VCtl.EPI_ESP*VCtl.EPI_ETL+CV8+CV5+CV4+2e-3;
p.tStart=VCtl.TE+VCtl.EPI_ESP*VCtl.EPI_ETL+CV8+CV5+CV4+1e-3;
if strcmp(p.Switch,'on')
[GxAmp9,GxTime9]=GxAreaTrapezoid(p);
GxAmp=[GxAmp GxAmp9];
...
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.tMiddle=VCtl.TE+0.5*VCtl.EPI_ESP*VCtl.EPI_ETL+CV8+CV5+0.5e-3;
if strcmp(p.Switch,'on')
...
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.tMiddle=VCtl.TE+1.5*VCtl.EPI_ESP*+VCtl.EPI_ETL+CV8+2.358e-3+CV5+CV7;
if strcmp(p.Switch,'on')
