...
%     set(Simuh.Attrh1.ResFreq,'String',num2str(VCtl.ResFreq - 1));
    set(Simuh.Attrh1.ResFreq,'String',num2str(VCtl.ResFreq));
    set(Simuh.Attrh1.SliceNum,'String',num2str(VCtl.SliceNum));
...
if strcmp(name,  '  ') 
%     dB0 = VMag.dB0;
%     diff_h = [abs(diff(dB0,1));zeros(1,size(dB0,2))];
%     diff_v = [abs(diff(dB0,1,2)),zeros(size(dB0,1),1)];
%     diff_dB0 = sqrt(diff_h.^2+diff_v.^2);
%     T2_o = 1./(VObj.Gyro*diff_dB0/2/3.14159+eps);
%     T2_o = (T2_o>100).*100+(T2_o<=100).*T2_o; % when setting to 100, the influence is less than 0.1%
%     VSig.T2STAR2 = 1./(1./(VObj.T2Star(:,:)+eps)+1./(T2_o(:,:)+eps)+eps);     % the T2star value due to B0 inhomogeneity
%     R2 = 1./(VObj.T2Star+eps)-1./(VObj.T2+eps)+1./(T2_o+eps);
%     dB0 = dB0(1:end-1,:);
    if VObj.SpinNum >1
        InddWRnd=linspace(0.01,0.99,VObj.SpinNum);
        for j=1:VObj.TypeNum
            for i=1:VObj.SpinNum
%                 VMag.dWRnd(:,:,:,i,j)=((1./(VObj.T2Star(:,:,:,j)+eps)-1./(VObj.T2(:,:,:,j)+eps)+1./(T2_o(:,:)+eps))).*tan(pi.*(InddWRnd(i)-1/2));
                VMag.dWRnd(:,:,:,i,j)=(1./(VObj.T2Star(:,:,:,j)+eps)-1./(VObj.T2(:,:,:,j)+eps)).*tan(pi.*(InddWRnd(i)-1/2));
...
if isfield(Simuh,'CoilTxXMLFile')
    ...
    if strcmp(VCtl.MultiTransmit,'off')
        if strcmp(name,  'Coil_WJG_B1')
            wjg_B1 = WJG_B1_rand();
            VCoi.TxCoilmg=abs(wjg_B1); % total B+ field magnitude after normalization
            VCoi.TxCoilpe=zeros(size(wjg_B1)); % total B+ field phase
            VCoi.TxCoilNum=1;
