...
disp('          _______           _______  _______ _________');
disp('|\     /|(       )|\     /|(       )(  ____ )\__   __/');
disp('( \   / )| () () || )   ( || () () || (    )|   ) (   ');
disp(' \ (_) / | || || || |   | || || || || (____)|   | |   ');
disp('  ) _ (  | |(_)| || |   | || |(_)| ||     __)   | |   ');
...
disp('Version 2020.01.07');
disp('Initializing ... ');
disp('!!!!!!!!!!!!!!Attention!!!!!!!!!! ');
disp('This software can only be used in our own group, should not be shared with others before the corresponding work are published!');
disp('You are responsible for your possible infingement act!');
disp('!!!!!!!!!!!!!!Attention!!!!!!!!!! ');
%MRiLab path
...
%         GPU_memory(i) = handles.GPUInfo.AvailableMemory;
%         toc
...
%     end
%     [~,select_GPU]=max(GPU_memory);
%     GPUhandle = ['handles.GPU',num2str(select_GPU),'_uimenu'];
%     Uimenu_ChkFunction([],[],eval(GPUhandle)); % Set first GPU as default processing unit

    if handles.GPUCount~=0
        for i=3 %:handles.GPUCount
            tic
            handles.GPUInfo=gpuDevice(i);
            GPU_memory(i) = handles.GPUInfo.AvailableMemory;
            toc
...
global VMot; VMot=[];
global VVar; VVar=[];
global WJG_frequency; WJG_frequency=0;		%CEST frequency
global WJG_ran_grad_array; WJG_ran_grad_array = zeros(2,20);    %initial the WJG_ran_grad_array
global WJG_Bvalue_idx; WJG_Bvalue_idx=1;
global LQ_TSL_idx; LQ_TSL_idx=1;    %lq
try
...
if length(tabs)>4 % CVs section
    ...
    if (isfield(VCtl,'CEST_Flag')&& strcmp(VCtl.CEST_Flag, 'on'))
        handles.SeqStruct.Children(1).Attributes(8).Value = num2str(WJG_frequency);
    end
...
global WJG_ran_grad_array;      %used for random gradient shift
set(handles.Scan_pushbutton,'Enable','off');
set(handles.Batch_pushbutton,'Enable','off');
set(hObject,'String','Init...');
pause(0.01); 
if (isfield(VCtl,'DEEP_learning')&&(VCtl.DEEP_learning==1))
         %%%%%%%%%%%%%%mulitislice
        %%%%modified by angus
    for loopj = VCtl.start:VCtl.end
        if isfield(VCtl,'rf_pro_consider')&&strcmp(VCtl.rf_pro_consider, 'on')
            VObj = WJ_gen_obj4ms(VObj.ZDim, loopj);
        else
            VObj = WJ_gen_obj(VObj.ZDim,loopj);
        end
        %% consider the random gradient shift
        WJG_ran_grad_array = randi([-100,100],2,20)*3/100/100; %0%cwk         
        ...
        try 
            ...
            Exe = 0;
            for i = 0:99 % check first 99 possible GPU device
                if isfield(handles,['GPU' num2str(i) '_uimenu'])
                    if strcmp(get(handles.(['GPU' num2str(i) '_uimenu']),'Checked'),'on')
                        ...
                        elseif isfield(VCtl, 'GM_Flag')
                            ...
                            set(hObject,'String','Scan...');
                            pause(0.01);
%                             figure(2);imshow(VCoi.TxCoilmg,[]);colormap jet
%                             figure(3);imshow(VCoi.TxCoilpe,[]);colormap jet
                            DoScanAtGPU;  % global (VSeq,VObj,VCtl,VMag,VCoi,VVar,VSig) are needed
                        end
                        Exe = 1;
                        break;
        ...
        %% data analysis avoid wrong data
        Sx = squeeze(VSig.Sx);
        Sy = squeeze(VSig.Sy);        
        K1 = Sx(1:end)+1i*Sy(1:end);  
        if sum(abs(K1(end-20:end)))==0
            disp ('wrong data1')
            break
        end
        save_file = [VCtl.output_path,'DEEP_Learning',num2str(loopj)];
        VSig.Mx = []; VSig.My = []; VSig.Mz = []; VSig.Kx = [];VSig.Ky = [];VSig.Kz = [];
        if isfield(VCtl,'output_T1')&&strcmp(VCtl.output_T1, 'on')
           VSig.T1 = single(VOex.T1);
        end
        if isfield(VCtl,'output_T2')&&strcmp(VCtl.output_T2, 'on')
           VSig.T2 = single(VOex.T2);
        end
        if isfield(VCtl,'output_T2STAR')&&strcmp(VCtl.output_T2STAR, 'on')
           VSig.T2STAR = single(VOex.T2Star);
        end
        if isfield(VCtl,'output_Rho')&&strcmp(VCtl.output_Rho, 'on')
           VSig.Rho = single(VOex.Rho);
        end
%         if isfield(VCtl,'output_B1')&&strcmp(VCtl.output_B1, 'on')
%             if  (isempty(WJG_Coi_B1))
%                 VSig.B1 = single(VCoi.TxCoilmg);
%             else
%                 VSig.B1 = WJG_Coi_B1;
%             end
%         end 
        ...
        if isfield(VCtl,'output_B2')&&strcmp(VCtl.output_B2, 'on')
           VSig.B1 = WJG_Coi_B1;
           VSig.B2 = WJG_Coi_B2;
        end  
        if isfield(VCtl,'output_B0')&&strcmp(VCtl.output_B0, 'on')
           VSig.B0 =  single(VMag.dB0);
        end          
        if isfield(VCtl,'output_ADC')&&strcmp(VCtl.output_ADC, 'on')
           VSig.ADC =  single(VOex.WJG_ADC);
        end  
        if isfield(VCtl,'output_T1rho')&&strcmp(VCtl.output_T1rho, 'on')
           VSig.T1rho =  single(VObj.T1rho);
        end 
        if isfield(VCtl,'output_T2rho')&&strcmp(VCtl.output_T2rho, 'on')
           VSig.T2rho =  single(VObj.T2rho);
        end  
        VSig.Sx = single(VSig.Sx);
        VSig.Sy = single(VSig.Sy);
%         VSig.m0_scale = VObj.m0_scale;
        save(save_file, 'VSig');
    end
elseif (isfield(VCtl,'CEST_Flag')&& strcmp(VCtl.CEST_Flag, 'on'))
         %%%%%%%%%%%%%%mulitislice
        %%%%modified by angus
        [WJG_row,WJG_col,WJG_slice] = size(VMag.FRange);
        ...
        for loopi =VCtl.frestart:VCtl.frestep:VCtl.freend
            ...
            try
                ...
                VMag = WJG_VMag;
                VVar = WJG_VVar;
                WJG_frequency = loopi*42.576*VCtl.B0;
                Save_pushbutton_Callback(hObject, eventdata, handles)
                DoPreScan(handles);
                DoScanSeriesUpd(handles,1);                
                % Preserve VObj VMag
