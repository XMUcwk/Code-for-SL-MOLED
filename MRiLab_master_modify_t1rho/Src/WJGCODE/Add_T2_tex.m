...
T2min = 10;%11changge to 5 cwk
%T2_temp = unifrnd(T2min,T2max);
r=rand();
if r<=0.10
T2_temp =rand(1,1)*10+5;
elseif r<=0.8
 T2_temp =rand(1,1)*60+15;
elseif r<=0.95
 T2_temp =rand(1,1)*40+75;
else
 T2_temp =rand(1,1)*200+115;
end %rat
%%%%%%%%%%%%%%%%T2STAR
T2STARmin = 3;
%T2STARmax = T2_temp;
...
s=rand(1,1);
if s<=0.4
T2STAR_temp =rand(1,1)*9+3;
elseif s<=0.85
 T2STAR_temp =rand(1,1)*40+12;
elseif s<=0.95
 T2STAR_temp =rand(1,1)*40+50;
 else
 T2STAR_temp =rand(1,1)*100+90;
end%rat
