...
SX=reshape(VSig.Sx,VCtl.ResFreq * VCtl.EPI_ETL,VCtl.EPI_ShotNum,VCtl.SliceNum,VCoi.RxCoilNum,VObj.TypeNum); % matlab col priority
SY=reshape(VSig.Sy,VCtl.ResFreq * VCtl.EPI_ETL,VCtl.EPI_ShotNum,VCtl.SliceNum,VCoi.RxCoilNum,VObj.TypeNum);
...
S1 = flipud(S1);
 final=S1;%%L/R
 for loopi=1:64
    each_line = ifftshift(S1(:,loopi));
    temp_ans =fftshift( fft(each_line));
    final(:,loopi) = temp_ans;
 end
 final = final';%%L/R
for i = 1: VCoi.RxCoilNum
    VImg.Real(:,:,:,i)=real(final);
    VImg.Imag(:,:,:,i)=imag(final);
