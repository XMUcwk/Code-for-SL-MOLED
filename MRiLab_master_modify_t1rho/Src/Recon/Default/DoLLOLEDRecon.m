...
length = VCtl.ResFreq * VCtl.EPI_ETL;
for loopi = 1:VCtl.LLOLED_Slice %multi epi
    Sx_t = VSig.Sx((loopi-1)*length+1:loopi*length);
    Sy_t = VSig.Sy((loopi-1)*length+1:loopi*length);
    SX=reshape(Sx_t,VCtl.ResFreq * VCtl.EPI_ETL,VCtl.EPI_ShotNum,VCtl.SliceNum,VCoi.RxCoilNum,VObj.TypeNum); % matlab col priority
    SY=reshape(Sy_t,VCtl.ResFreq * VCtl.EPI_ETL,VCtl.EPI_ShotNum,VCtl.SliceNum,VCoi.RxCoilNum,VObj.TypeNum);

    SX=sum(SX, 5); % sum signal from all spin types
    SY=sum(SY, 5);
    %% EPI k-space recon
    % sort k-space & row flipping
    Sx=zeros(VCtl.ResFreq,VCtl.EPI_ETL*VCtl.EPI_ShotNum,VCtl.SliceNum,VCoi.RxCoilNum);
    Sy=zeros(VCtl.ResFreq,VCtl.EPI_ETL*VCtl.EPI_ShotNum,VCtl.SliceNum,VCoi.RxCoilNum);
    for e = 1: VCtl.EPI_ETL
        if (-1)^(e-1)==1
            Sx(:,(e-1)*VCtl.EPI_ShotNum+1: e*VCtl.EPI_ShotNum, :,:) = reshape(SX((e-1)*VCtl.ResFreq+1: e*VCtl.ResFreq, :,:,:), [VCtl.ResFreq ,VCtl.EPI_ShotNum ,VCtl.SliceNum,VCoi.RxCoilNum]);
            Sy(:,(e-1)*VCtl.EPI_ShotNum+1: e*VCtl.EPI_ShotNum, :,:) = reshape(SY((e-1)*VCtl.ResFreq+1: e*VCtl.ResFreq, :,:,:), [VCtl.ResFreq ,VCtl.EPI_ShotNum ,VCtl.SliceNum,VCoi.RxCoilNum]);
        else % row flipping
            Sx(:,(e-1)*VCtl.EPI_ShotNum+1: e*VCtl.EPI_ShotNum, :,:) = reshape(flipdim(SX((e-1)*VCtl.ResFreq+1: e*VCtl.ResFreq, :,:,:),1), [VCtl.ResFreq ,VCtl.EPI_ShotNum ,VCtl.SliceNum,VCoi.RxCoilNum]);
            Sy(:,(e-1)*VCtl.EPI_ShotNum+1: e*VCtl.EPI_ShotNum, :,:) = reshape(flipdim(SY((e-1)*VCtl.ResFreq+1: e*VCtl.ResFreq, :,:,:),1), [VCtl.ResFreq ,VCtl.EPI_ShotNum ,VCtl.SliceNum,VCoi.RxCoilNum]);
        end
    end
...
%     ResFreq = VCtl.ResFreq - 1;
    ...
    if isfield(VCtl,'ZF_Kx') % Zero Filling of k-space for increasing matrix size, no increae of resolution
        ...
        for i = 1:VCoi.RxCoilNum
            Sx2(str2double(VCtl.ZF_Kx)/2-ResFreq/2+1:str2double(VCtl.ZF_Kx)/2-ResFreq/2+1+ResFreq-1, ...
                str2double(VCtl.ZF_Ky)/2-(VCtl.EPI_ETL*VCtl.EPI_ShotNum)/2+1:str2double(VCtl.ZF_Ky)/2-(VCtl.EPI_ETL*VCtl.EPI_ShotNum)/2+1+(VCtl.EPI_ETL*VCtl.EPI_ShotNum)-1, ...
    ...
    VImg.Sx(:,:,:,loopi)=Sx;
    VImg.Sy(:,:,:,loopi)=Sy;
%     figure(7);imshow(VImg.Mag,[]);colormap jet
%     figure(8);imshow(abs(S),[]);colormap jet
