...
clear 
close all
dirname='/data4/mlc/datagen90/1002Hz/';
outputdir = [dirname, '/data4trainccb/'];
...
max_amp =  max_amp_I1(dirname);  % 4.5043 for the datagen50
...
%         I2 = fliplr(I2);
        ...
        figure(3);imshow((B0.*mask/50),[-1,1]);colormap jet;
%         figure(4);imshow(abs(M0),[]);colormap jet;
% %         figure(5);imshow(WJG_edge,[]);colormap jet;colorbar
% %         figure(6);imshow(ychange,[]);colormap jet;colorbar
        figure(7);imshow(T2*2,[]);colormap jet;
%         figure(8);imshow(T2STAR*3,[]);colormap jet;
...
%         figure(10);imshow(abs(ifftshift(fft2(fftshift(I2)))),[]);colormap jet;
        ...
        output(3,:,:) = real(I2);
        output(4,:,:) = imag(I2);
        output(5,:,:) = abs(ychange);
        output(6,:,:) = abs(WJG_edge);
        output(7,:,:) = T2*2;
        output(8,:,:) = T2STAR*3;
        output(9,:,:) = B0.*mask/50;
        output(10,:,:) = M0;
        ...
        filename=[outputdir,'brain_',num2str(WJG_order),'.Charles'];
