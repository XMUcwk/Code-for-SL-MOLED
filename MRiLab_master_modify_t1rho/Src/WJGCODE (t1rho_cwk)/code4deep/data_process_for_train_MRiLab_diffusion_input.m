...
clear 
close all
dirname='/data2/mlc/MRiLab/brian_phantom/test0720_770_mlc/';
outputdir = ['/data2/mlc/MRiLab/brian_phantom/test0720_770/'];
fid_dir_all=dir([dirname,'*']);       %list all directories
phase_num=126;
fre_num = 128;
expand_num =128;
sigma=0.6e-2;
WJG_order=1;
max4norm =  2.6;%max_amp_I1(dirname);
...
% %         figure(7);imshow(B1.*mask,[]);colormap jet;colorbar
%         figure(8);imshow(ADC*1000,[]);colormap jet;
        figure(9);imshow(log((abs(ifftshift(fft2(fftshift(I1)))))+1e-6),[]);colormap jet;
        figure(10);imshow(log((abs(ifftshift(fft2(fftshift(I2)))))+1e-6),[]);colormap jet;
        output(1,:,:) = real(I1);
        output(2,:,:) = imag(I1);
        output(3,:,:) = real(I2);
        output(4,:,:) = imag(I2);
        ...
        outputt=permute(output,[2 3 1]);
        figure(1);imshow(abs(outputt(:,:,1)),[]);colormap jet;
        figure(2);imshow(abs(outputt(:,:,2)),[]);colormap jet;
        figure(6);imshow(outputt(:,:,5),[]);colormap jet;
%         figure(7);imshow(B1.*mask,[]);colormap jet;colorbar
        figure(8);imshow(outputt(:,:,6),[]);colormap jet;
